...
$("#confirmDefaultSeance").on("click", function () {
    let values = $('input[name="Semaines"]:checked');

    for (let i = 0; i < values.length; i += 4) {
        let monthDate = values[i]["value"].split("-")[0];
        let year = getAllDaysInMonth(2022, parseInt(monthDate));
    }
});
const targetDiv = document.getElementById("checkHide");
var count = 0;
function semaineDefaut(args) {
    if (count == 0) {
...
function postDay(info) {

    let day = new Date(info.event.startStr).getDay();
}
...
(function ($, document, window) {
    ...
    var pluginObject = {
        init: function (opts, support, container) {
            ...
            // self.container.addClass('monthDefaultSeance');
        ...
        selectStart: function (e) {
            ...
            self.startPoint =
                e.type === "keydown" ? self.lastChecked : $(e.target);
            ...
            self.bind(
                self.container,
                "mousemove.line touchmove.line",
                self.moveLine
            );
            self.bind(
                self.container,
                "mousemove.edge touchmove.edge",
                self.scrollOnEdge
            );
            self.bind(self.container, "touchmove", self.touchDragActions);
            self.bind(
                self.checkboxes,
                "mouseenter touchstart.second",
                self.dragActions
            );
        },
        ...
        dragActions: function (e) {
            ...
            if (e.type === "touchstart") {
                if (e.originalEvent.touches.length !== 1) {
                    ...
                    self.bind(self.container, "touchmove", function (e) {
                        e.preventDefault();
                    });
                } else {
                    return;
            ...
            self.bind(
                self.endPoint,
                "mouseup.select touchend.select",
                self.toggleCheckboxesRange,
                true
            );
        ...
        touchDragActions: function (e) {
            ...
            if (target) {
                ...
                if (
                    !self.onTouchLeave &&
                    !(self.endPoint && target === self.endPoint[0]) &&
                    target !== self.container[0] &&
                    self.container[0].contains(target)
                ) {
        ...
        clean: function (e) {
            ...
            self.container.find(".checkbox-range-point");
            self.svgLine
                .attr("x1", 0)
                .attr("y1", 0)
                .attr("x2", 0)
        ...
        bind: function (element, events, method, one) {
            ...
            if (/ /.test(events)) {
                ...
                var eventsNamespaced =
                    eventsArray.join(namespace + " ") + namespace;
            } else {
        ...
        assembleMarkElements: function () {
            ...
            self.svgCanvas = $(
                '<svg class="checkbox-range-bound-canvas"></svg>'
            );
        ...
        updateLineStart: function () {
            ...
            var x =
                self.startPoint.offset().left -
                self.containerOffLeft +
                self.opts.lineOffsetLeft;
            var y =
                self.startPoint.offset().top -
                self.containerOffTop +
                self.opts.lineOffsetTop;
            self.svgLine
                .attr("x1", x)
                .attr("y1", y)
                .attr("x2", x)
            ...
            if (!self.stopMoveLine) {
                ...
                switch (self.opts.path) {
                    case "horizontal":
                        ...
                        var y =
                            self.startPointOffTop -
                            self.containerOffTop +
                            self.opts.lineOffsetTop;
                        break;
                    case "vertical":
                        var x =
                            self.startPointOffLeft -
                            self.containerOffLeft +
                            self.opts.lineOffsetLeft;
        ...
        createOnTouchLabels: function () {
            ...
            self.checkboxes.each(function () {
                var checkbox = $(this);
                var label =
                    checkbox.siblings("label")[0] ||
                    checkbox.parent("label")[0];
                if (label) {
                    ...
                    if (labeltxt.length > self.opts.onTouchLabelsLimit) {
                        labeltxt =
                            labeltxt.substring(
                                0,
                                self.opts.onTouchLabelsLimit - 1
                            ) + "...";
                    }
                    checkbox
                        .parent()
                        .append(
                            '<span class="ontouch-label">' +
                                labeltxt +
                                "</span>"
                        );
                }
        ...
        magnetToEndPoint: function () {
            ...
            var x =
                self.endPoint.offset().left -
                self.containerOffLeft +
                self.opts.lineOffsetLeft;
            var y =
                self.endPoint.offset().top -
                self.containerOffTop +
                self.opts.lineOffsetTop;
            self.svgLine.attr("x2", x).attr("y2", y);
        },
        ...
        getElementsRange: function () {
            ...
            self.checkboxes.each(function (index) {
                if (
                    index > self.startPointIndex &&
                    index < self.endPointIndex
                ) {
    ...
    $.fn[pluginName] = function (options) {
        ...
        var support = {
            stContainer: /AppleWebKit/.test(navigator.userAgent)
                ? document.body
        ...
        return this.each(function () {
            ...
            if (!pluginInstance) {
                pluginInstance = Object.create(pluginObject).init(
                    opts,
                    support,
                    this
                );
